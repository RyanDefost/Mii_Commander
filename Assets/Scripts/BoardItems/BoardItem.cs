...
namespace BoardItems
{
    ...
    private void Start()
    {
        ...
        {
            if (this.OnActivate == null) return;
            foreach (Delegate del in this.OnActivate.GetInvocationList())
            {
                if (del.Target is not BoardItemComponent boardItemComponent) continue;
                string abilityName = boardItemComponent.GetAbilityName();
                if (string.IsNullOrEmpty(abilityName)) continue;
                Helper.DoGlobalCallItemActivated(this.gameObject.name, abilityName);
            }
            this.OnActivate();
        }
